...
@lexer::members {this.parenthesis = []; 
	reset(){
		super.reset();
		this.parenthesis = [];
	}
...
LCURLY: '{';
LPAREN:
	'(' { if (this.parenthesis.length > 0) this.parenthesis.push(this.parenthesis.pop() + 1);};
LBRACK: '[';
RCURLY: '}';
RPAREN:
	')' { if(this.parenthesis.length > 0) {
			this.parenthesis.push(this.parenthesis.pop() - 1); 
			if (this.parenthesis[this.parenthesis.length - 1] == 0) { 
...
Interpolataion_single_line:
	'\\(' { this.parenthesis.push(1);} -> pushMode(DEFAULT_MODE);

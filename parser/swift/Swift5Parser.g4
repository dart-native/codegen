...
@header {import SwiftSupport from './SwiftSupport.js' }
@members {this.swiftSupport = new SwiftSupport();}
...
statements
	locals[int indexBefore = -1]: (
		{this.swiftSupport.isSeparatedStatement(this._input, $indexBefore)}? statement {$indexBefore = this._input.index;
			}
	)+;
...
balanced_token_punctuation:
	...
	| arrow_operator
	| {this.swiftSupport.isPrefixOp(this._input)}? AND
	| {this.swiftSupport.isPostfixOp(this._input)}? BANG;
...
forced_value_suffix: {!this.swiftSupport.isBinaryOp(this._input)}? BANG;
optional_chaining_suffix:
	{!this.swiftSupport.isBinaryOp(this._input)}? QUESTION;
...
type:
	...
	| opaque_type
	| type (
		{!this.swiftSupport.isBinaryOp(this._input)}? QUESTION //optional_type
		| {!this.swiftSupport.isBinaryOp(this._input)}? BANG //implicitly_unwrapped_optional_type
		| DOT TYPE
		| DOT PROTOCOL
	) //metatype_type
	| any_type
...
trailing_composition_and:
	{!this.swiftSupport.isBinaryOp(this._input)}? AND;
...
// Assignment Operator
assignment_operator: {this.swiftSupport.isBinaryOp(this._input)}? EQUAL;

negate_prefix_operator: {this.swiftSupport.isPrefixOp(this._input)}? SUB;

compilation_condition_AND:
	{this.swiftSupport.isOperator(this._input,"&&")}? AND AND;
compilation_condition_OR:
	{this.swiftSupport.isOperator(this._input,"||")}? OR OR;
compilation_condition_GE:
	{this.swiftSupport.isOperator(this._input,">=")}? GT EQUAL;
compilation_condition_L: {this.swiftSupport.isOperator(this._input,"<")}? LT;
arrow_operator: {this.swiftSupport.isOperator(this._input,"->")}? SUB GT;
range_operator: {this.swiftSupport.isOperator(this._input,"...")}? DOT DOT DOT;
same_type_equals:
	{this.swiftSupport.isOperator(this._input,"==")}? EQUAL EQUAL;

binary_operator: {this.swiftSupport.isBinaryOp(this._input)}? operator;

prefix_operator: {this.swiftSupport.isPrefixOp(this._input)}? operator;

postfix_operator: {this.swiftSupport.isPostfixOp(this._input)}? operator;
...
operator_characters: (
		{this._input.get(this._input.index-1).getType()!=WS}? operator_character
	)+;

dot_operator_head: DOT;
dot_operator_character: DOT | operator_character;
dot_operator_characters: (
		{this._input.get(this._input.index-1).getType()!=WS}? dot_operator_character
	)+;

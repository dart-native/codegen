...
class DNObjectiveCParserListener extends ObjectiveCParserListener {
    ...
    enterEnumSpecifier(ctx) {
        if (this.currentContext instanceof DNEnumDefContext) {
            this.currentContext.type = ctx.children[0].symbol.text
        ...
        if (this.currentContext instanceof DNEnumDefContext) {
            ...
            ctx.children.forEach(element => {
                ...
                if (element.children.length == 1) {
                    ...
                    lastValue = element.children[2].children.map((c) => {
    ...
    enterOsVersion(ctx) {
        var osVersion = new DNOSVersionContext(ctx)
        this.currentContext.osVersions.push(osVersion)
    };

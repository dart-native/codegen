...
class DNObjectiveCParserListener extends ObjectiveCParserListener {
    ...
    enterBlockParameters(ctx) {
        if (this.currentContext instanceof DNArgumentContext) {
            var blockArgs = ctx.types.map((type) => {
                // FIXME: parse type correctly
                return type.start.text + ' ' + type.stop.text
            }).join(', ')
            blockArgs = '(' + blockArgs.substring(0, blockArgs.length) + ')'
    ...
    enterTypeSpecifier(ctx) {
        ...
        var isPointer2Pointer = false
        if (pointer) {
            if (type == 'void' || type == 'char') {
                type += ' *'
            }
            if (pointer.nextPointer) {
                isPointer2Pointer = true
            }
        }
        ...
        if (this.currentContext instanceof DNMethodContext) {
            ...
            this.currentContext.returnType = TC.convert(returnType, true)
            // FIXME: pointer to struct
            if (!DNObjectiveCTypeConverter.basicAutoConvertReturnTypes.includes(this.currentContext.returnType) &&
                pointer) {
                this.currentContext.callFromPointer = true
            }
        } else if (this.currentContext instanceof DNArgumentContext) {
            if(!this.currentContext.isBlock) {
                this.currentContext.type = TC.convert(type, true)
            }
            // case：(NSError ** )error
            this.currentContext.isOutParam = isPointer2Pointer
        } else if (this.currentContext instanceof DNBlockDefContext) {
            this.currentContext.returnType = TC.convert(type)
        }
    }
    ...
    enterEnumSpecifier(ctx) {
        if (this.currentContext instanceof DNEnumDefContext) {
            if (ctx.type) {
                this.currentContext.type = ctx.type.start.text
            }

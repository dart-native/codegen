...
class DNObjectiveCParserListener extends ObjectiveCParserListener {
    ...
    enterFunctionPointer(ctx) {
        ...
        if (this.currentContext instanceof DNMethodContext) {
            this.currentContext.returnType = 'Pointer<Function>'
        } else if (this.currentContext instanceof DNArgumentContext) {
            this.currentContext.type = 'Pointer<Function>'
        }
    };
    ...
    enterTypeSpecifier(ctx) {
        if (this.currentContext instanceof DNMethodContext) {
            ...
            let stop = ctx.stop.text
            this.currentContext.retValIsObj = (stop == '*' || stop == 'id')
        } else if (this.currentContext instanceof DNArgumentContext) {
            this.currentContext.type = TC.convert(ctx.start.text, true)
            if (ctx.children.length > 1) {
                var pointer = ctx.children[1]

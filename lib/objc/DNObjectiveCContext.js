...
class DNMethodContext extends DNContext {
    ...
    preHandleMutableArgsIfNeed(){
        ...
        this.args.forEach((element) => {
            if(DNMethodContext.SupportMutableTypes.indexOf(element.type) > -1){
                var tmpArgName = '_' + element.name
                result += '    ' + element.type + ' ' + tmpArgName + ' = ' + element.type + '(' + element.name +')\n' 
    ...
    convertMutableTypeIfNeed(type){
        var dartType = DNMethodContext.SupportMutableTypesMap[type]
        return dartType ? dartType : type
    }
}

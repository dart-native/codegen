...
export class DNEnumItemContext extends DNContext {
    ...
    parse() {
        ...
        if (this.type) {
            result += 'const ' + this.type + ' ' + this.name.camelize() + ' = ' + this.value + ';\n'
        } else {
            result += 'const int ' + this.name.camelize() + ' = ' + this.value + ';\n'
        }
        return result
    }
}
...
export class DNMethodContext extends DNContext {
    ...
    methodImpl(noArg) {
        ...
        var impl = callerPrefix + 'performSync(SEL(\'' + this.ocMethodName() + '\')' + args + ');\n'
    ...
    parse() {
        ...
        if (!this.isReadOnly) {
            var set = annotation + '  ' + 'set ' + this.name + '(' + convertedType + ' ' + this.name + ')' +
                ' =>' + isClassPrefix + 'performSync(SEL(\'set' + this.name.replace(/^\w/, c => c.toUpperCase()) + ':\'), args: [' + this.name + ']);'
        }
        return get + '\n' + set
    }

    handleGetter(isClassPrefix) {
        var impl = isClassPrefix + 'performSync(SEL(\'' + this.name + '\')' + ');\n'

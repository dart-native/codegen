...
import DNSwift5Converter from './swift/DNSwift5Converter.js'
...
const scriptForSwift5 = './swift/DNSwift5Converter.js'

const langForExtension = { h: 'objc', java: 'java', swift: 'swift' }
const scriptForExtension = {
    h: path.resolve(__dirname, scriptForObjC),
    java: path.resolve(__dirname, scriptForJava),
    swift: path.resolve(__dirname, scriptForSwift5),
}
...
function generateDartWithWorker(content, path, script) {
    return new Promise((resolve, reject) => {
        if (isMainThread) {
            ...
            } else if (script == scriptForExtension.swift) {
                converter = new DNSwift5Converter()
            }

import Swift5ParserListener from '../../parser/swift/Swift5ParserListener.js'
import Swift5Parser from '../../parser/swift/Swift5Parser.js'
import { DNRootContext, DNImportContext, DNTypealiasContext, DNClassContext, DNExtensionContext, DNProtocolContext, DNFunctionContext, DNProtocolMethodContext, DNPropertyContext, DNArgumentContext, DNObjcAttributeContext } from './DNSwift5Context.js'
import { fullTypeNameForType_identifierContext } from './DNSwift5ParserHelper.js'

// This class defines a complete listener for a parse tree produced by Swift5Parser.
export default class DNSwift5ParserListener extends Swift5ParserListener {
    ...
    enterConstant_declaration(ctx) {
        if (this.currentContext instanceof DNClassContext) {
            var isObjc = this.currentContext.isObjcMembers
            var property = new DNPropertyContext(ctx, isObjc)
    ...
    enterVariable_declaration(ctx) {
        if (this.currentContext instanceof DNClassContext) {
            var isObjC = this.currentContext.isObjcMembers
            var property = new DNPropertyContext(ctx, isObjC)
            if (ctx.pattern_initializer_list()) {
    ...
    enterVariable_name(ctx) {
        if (this.currentContext instanceof DNPropertyContext) {
            this.currentContext.name = ctx.start.text
        }
    }
    ...
    enterSetter_clause(ctx) {
        if (this.currentContext instanceof DNPropertyContext) {
            this.currentContext.isReadOnly = false
        }
    }
    ...
    enterFunction_declaration(ctx) {
        if (this.currentContext instanceof DNClassContext) {
            var isObjc = this.currentContext.isObjcMembers
            var method = new DNFunctionContext(ctx, isObjc)
    ...
    exitFunction_declaration(ctx) {
        if (this.currentContext instanceof DNFunctionContext) {
            var method = this.currentContext
            this.currentContext = this.currentContext.parent
            this.currentContext.properties.push(method)
        }
    }
    ...
    enterClass_declaration(ctx) {
        var classDeclaration = new DNClassContext(ctx)
        this.currentContext.addChild(classDeclaration)
        this.currentContext = classDeclaration
    }
    ...
    enterDeclaration_modifier(ctx) {
        ...
        if (this.currentContext instanceof DNFunctionContext) {
            if (modifier == 'static') {
                this.currentContext.isClassMethod = true
            }
        }
    }
    ...
    enterIdentifier_pattern(ctx) {
        const identifier = ctx.identifier().start.text
        if (this.currentContext instanceof DNPropertyContext ||
            this.currentContext instanceof DNArgumentContext) {
            this.currentContext.name = identifier
        }
    }
    ...
    enterAttribute(ctx) {
        let attributeName = ctx.attribute_name().start.text
        if (attributeName == 'objc') {
            var objcAttribute = new DNObjcAttributeContext(ctx)
            this.currentContext.addChild(objcAttribute)
            this.currentContext = objcAttribute
        }
        if (attributeName == 'objcMembers' && this.currentContext instanceof DNClassContext) {
            this.currentContext.isObjcMembers = true
        }
    }

    // Exit a parse tree produced by Swift5Parser#attribute.
    exitAttribute(ctx) {
        if (this.currentContext instanceof DNObjcAttributeContext) {
            var objcAttribute = this.currentContext
            this.currentContext = this.currentContext.parent
            this.currentContext.objcAttribute = objcAttribute
        }
    }
    ...
    enterAttribute_argument_clause(ctx) {
        if (this.currentContext instanceof DNObjcAttributeContext) {
            this.currentContext.objcName = ctx.balanced_tokens().balanced_token()[0].start.text
        }
    }
    ...
    enterType_identifier(ctx) {
        if (this.currentContext instanceof DNPropertyContext) {
            this.currentContext.type = fullTypeNameForType_identifierContext(ctx)
        }
    }
    ...
    enterArray_type(ctx) {
        const typeName = 'NSArray'
        if (this.currentContext instanceof DNPropertyContext ||
             this.currentContext instanceof DNArgumentContext) {
            this.currentContext.type = typeName
        }
    }
    ...
    enterType_inheritance_list(ctx) {
        if (this.currentContext instanceof DNClassContext) {
            let typeList = ctx.type_id_list.map(type_identifier => fullTypeNameForType_identifierContext(type_identifier))
            for (let index = 0; index < typeList.length; index++) {
                const element = typeList[index];
                if (index == 0) { // superClass
                    this.currentContext.superClass = element
                } else { // protocols
                    this.currentContext.protocols.push(element)
                }
            }
        }
    }
